...
def _log_audit(
    ...
    """Registra una entrada en audit_log para acciones sobre eventos. No bloquea."""
    try:
...
class EventoCardsReq(BaseModel):
    cliente_nombre: Optional[str] = None
    ...
    with conn.cursor() as cur:
        ...
        for sv in servicios:
            if not isinstance(sv, dict):
                continue
            id_servicio = sv.get("id_servicio")
            if not id_servicio:
                continue
            fecha = sv.get("fecha")           # "YYYY-MM-DD"
            hora_inicio = sv.get("hora_inicio")  # "HH:MM"
            hora_final  = sv.get("hora_final")   # "HH:MM"
            ...
            cur.execute(
                ...
                (
                    ...
                    hora_inicio,
                    hora_final,
                    sv.get("lugar") or None,
                    sv.get("id_paquete") or None,
                    id_user,
                    id_cliente,
                    sv.get("id_ciudad") or None,
                    sv.get("comentarios") or None,
                    int(id_servicio),
                ),
            )
    ...
    for r in rows:
        ...
        if fecha_evento is not None:
            try:
                ...
                fecha_str = str(fecha_evento)[:10] or None

        def _hora_to_str(val):
            if val is None:
                return None
            try:
                # timedelta (lo que mysql-connector regresa para columnas TIME)
        ...
        result.append({
            ...
            "hora_final": _hora_to_str(r.get("hora_final")),
            "lugar": r.get("lugar"),
            "id_ciudad": r.get("id_ciudad"),
            "id_paquete": r.get("id_paquete"),
...
async def crear_evento(
    ...
    allowed_fields = EventoCreate.model_fields.keys()
    basic_data = {k: v for k, v in payload_dict.items() if k in allowed_fields}
    payload = EventoCreate(**basic_data)
...
async def importar_excel_eventos(
    file: UploadFile = File(...),
    current_user: Dict[str, Any] = Depends(get_current_user),
):
    import io
...
async def upload_documento(
    ...
    tipo_label = "evento principal" if id_tipo_documento == 1 else "documento adicional"
    _log_audit(
        "DOCUMENTO_ADD",
