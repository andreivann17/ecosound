// src/components/laboral/PrimeraAudienciaView.jsx
...
const DocumentOpenBox = ({
  ...
  icon = "picture_as_pdf", // material symbol
  badgeText = null, // ej: "Cumplimiento de Convenio"
  metaLeft = null,  // ej: "3.4 MB"
  metaRight = null, // ej: "Finalizado"
  id,
}) => {
...
function PrimeraAudienciaView({ data, header, onEdit, abogadoIndex }) {
  const { Dragger } = Upload;
  const [constForm] = Form.useForm();
  const expedienteVal = header?.expediente || null;
  const dispatch = useDispatch();


const [constanciaFileList, setConstanciaFileList] = useState([]);

  // OJO: constanciaDoc depende de data, así que lo declaramos después de validar data.
  // Para no romper hooks, validamos data con un fallback:
  const safeData = data || {};
  ...
  // YA DESPUÉS haces el early return, pero SIN hooks debajo
  if (!data) return null;
  ...
  const documentoPreviews = Array.isArray(documento?.previews)
    ? documento.previews
    : [];
    // === Constancia de cumplimiento (segundo PDF) ===


  const constanciaRecibida = data.is_constancia_documento == 1 && !!constanciaDoc;
  ...
  // fecha: primero usa fecha_constancia_documento, si no, created_at del doc
  const constanciaFecha =
    data.fecha_constancia_documento || constanciaDoc?.created_at || null;
  ...
  const prestacionesRawPrimary = Array.isArray(data.prestaciones)
    ? data.prestaciones
    : null;
  const showConstanciaColumn = resultado === "convenio";

  const showDocumentoCard =
    hasDocumentoPreview || (showConstanciaColumn && hasConstanciaPreview);
...
const handleGuardarCumplimientoView = async () => {
  try {
    ...
    await dispatch(
      actionConstanciaCumplimientoCreate(payload, (resp) => {
        ...
        // si tu backend regresa path/preview/filename, actualizas el state local
        // (esto es opcional y depende del response real)
  ...
  // =========================
  // Flags de visibilidad por resultado
  // =========================

  // Objeto de reclamación:
  // - Se muestra en: no_conciliacion, diferimiento
  // - No se muestra en: archivo_solicitante, archivo_patron, incomparecencia, convenio
  ...
  // - Se muestra en: no_conciliacion, diferimiento
  // - No se muestra en: convenio, archivo_solicitante, archivo_patron, incomparecencia
      ...
      <Row justify="space-between" align="middle" style={{ marginBottom: 24 }}>
        <Col>
          <Title level={4} style={{ marginBottom: 6 }}>
            Primera audiencia prejudicial
          </Title>
          <Text type="secondary">
            Resumen completo de la primera sesión de negociación.
          </Text>
                ...
                <table className="audiencia-table resumen">
                  <thead>
                    <tr>
                      ...
                      <th style={{ width: "24%" }}>Forma de pago</th>
                      <th style={{ width: "24%" }}>Tipo de pago</th>
                    </tr>
                  </thead>
                  <tbody>
  ...
  return (
    <div className="aud-doc-card is-constancia">
      <div className="aud-doc-card__row">
        ...
        <div className="aud-doc-card__content">
          {/* ====== HEADER (solo si YA hay constancia) ====== */}
          {hasConstancia && (
            <>
              <div className="aud-doc-card__head">
                <div className="aud-doc-card__title">Constancia de Cumplimiento</div>
              ...
              <div className="aud-doc-card__chips">
                <span className="aud-doc-card__chip">
                  {constanciaDocState?.size_mb ? `${constanciaDocState.size_mb} MB` : "—"}
                </span>
                <span className="aud-doc-card__chip">
              ...
              <div className="aud-doc-card__actions">
                <button
                  ...
                  onClick={() => openFile(pickDoc(constanciaDocState).openUrl,header.id)}
                >
          ...
          {/* ====== FORM (solo si NO hay constancia) ====== */}
          {!hasConstancia && (
            <div className="aud-const-form aud-const-form--only">
              <div className="aud-const-form__title">Cumplimiento de Convenio</div>

              <Form
                form={constForm}
                layout="vertical"
                initialValues={{ estado_cumplimiento: "completo" }}
              >
                <Form.Item label="Cargar constancia de cumplimiento">
                  ...
                  >
                    ...
                    <p style={{ fontWeight: 600 }}>Subir un archivo o arrastrar y soltar</p>
                    <p style={{ color: "#6b7280", fontSize: 12 }}>PDF, PNG, JPG hasta 10MB</p>
                  </Dragger>
                </Form.Item>
                ...
                <Row gutter={[12, 0]}>
                  ...
                  <Col xs={24} md={12}>
                    <Form.Item label="Notas de cumplimiento" name="notas_cumplimiento">
                      <Input.TextArea rows={3} placeholder="Describe detalles adicionales..." />
                    </Form.Item>
                  </Col>
                </Row>

                <div className="aud-const-form__actions">
                  <Button onClick={() => constForm.resetFields()}>Cancelar</Button>
                  <Button
                    type="primary"
        ...
        {/* RIESGOS (siempre se muestran, cualquier resultado) */}
        <Card
          size="small"
          type="inner"
          title=" Riesgos"
...
export default PrimeraAudienciaView;
